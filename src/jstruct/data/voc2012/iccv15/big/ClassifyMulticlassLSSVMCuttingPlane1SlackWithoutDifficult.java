...
public class ClassifyMulticlassLSSVMCuttingPlane1SlackWithoutDifficult {
	...
	public static void main(String[] args) {
		...
		double[] lambdaCV = {Double.parseDouble(args[2])};
		//double[] epsilonCV = {1e-2, 1e-3};
		//double[] epsilonCV = {1e-3};
		double[] epsilonCV = {Double.parseDouble(args[3])};
		...
		String trainset = args[0];//"train";
		String testset = args[1];//"val";
				...
				for(double epsilon : epsilonCV) {
					for(double lambda : lambdaCV) {
						...
						if(fileClassifier != null) {
							...
							n++;
							double ap = 0;
							//System.err.println(tm[n]);
							if(tm[n].compareToIgnoreCase("NaN") == 0) {
								ap = -1;
							}
							else {
								ap = Double.parseDouble(tm[n]);
							}
							if(ap > map[s][split]) {
								map[s][split] = ap;
							}
