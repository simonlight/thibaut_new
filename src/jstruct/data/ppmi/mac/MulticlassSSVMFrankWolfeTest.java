...
import fr.durandt.jstruct.ssvm.multiclass.DoubleMulticlassSSVMFrankWolfe;
import fr.durandt.jstruct.struct.STrainingSample;
import fr.lip6.jkernelmachines.type.TrainingSample;

public class MulticlassSSVMFrankWolfeTest {
	...
	public static String simDir = "/Volumes/Eclipse/LIP6/simulation/PPMI/";
		...
		for(int scale : scaleCV) {
			...
			for(double epsilon : epsilonCV) {
				for(double lambda : lambdaCV) {
					...
					DoubleMulticlassSSVMFrankWolfe svm = new DoubleMulticlassSSVMFrankWolfe(); 
			...
			if(compute) {
				List<TrainingSample<BagMIL>> listTrain = BagReader.readBagMIL(inputDir + "/" + cls + "_" + features + "_train_scale_" + scale + ".txt", numWords);
				...
				List<TrainingSample<BagMIL>> listTest = BagReader.readBagMIL(inputDir + "/" + cls + "_" + features + "_test_scale_" + scale + ".txt", numWords);
