 ...
 *
 */
public class CombinaisonObjectBankLinearMantraM2CVPlatt {
				...
				for(int split : splitCV) {
					...
					for(double a : aCV) {
						for(double b : bCV) {
							...
							acc[split-1] = Math.max(scoreMax, acc[split-1]);
						}
					}
				}

				System.out.println("Accuracy");
				System.err.println("lambda= " + lambda + "\tepsilon= " + epsilon + "\tacc= " + VectorOp.mean(acc) + "\tmax= " + max);

				max = Math.max(max, VectorOp.mean(acc));
				System.err.println("lambda= " + lambda + "\tepsilon= " + epsilon + "\tacc= " + VectorOp.mean(acc) + "\tmax= " + max);
			}
		}
