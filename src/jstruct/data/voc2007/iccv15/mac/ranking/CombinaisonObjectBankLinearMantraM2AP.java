...
import fr.durandt.jstruct.ssvm.ranking.DoubleRankAPSSVMPegasos;
 ...
 *
 */
public class CombinaisonObjectBankLinearMantraM2AP {
	...
	public static void main(String[] args) {
		...
		//double[] cCV = {1e-4};
		int[] epochsCV = {10, 50, 100, 200, 500};
		//int[] epochsCV = {50};
		...
		for(int split : splitCV) {
			...
			String scoreDir = simDir + "/ICCV15/scoresMaxMin/MANTRA/M2/AP/CuttingPlane1Slack/BagImageRegion/";
			...
			List<STrainingSample<double[],Integer>> finalScoresTrain = null;
			List<STrainingSample<double[],Integer>> finalScoresTest = null;

			finalScoresTrain = STrainingSample.trainingSample2STrainingSample(concatenation(listScoresTrain, listTrain));
			finalScoresTest = STrainingSample.trainingSample2STrainingSample(concatenation(listScoresTest, listTest));

			RankingInput rankTrain = new RankingInput(finalScoresTrain);
			RankingOutput rankTrainY = new RankingOutput(finalScoresTrain);
			...
			RankingInput rankTest = new RankingInput(finalScoresTest);
			RankingOutput rankTestY = new RankingOutput(finalScoresTest);
			...
			for(double c : cCV) {
				for(int epochs : epochsCV) {
					...
					DoubleRankAPSSVMPegasos classifierRank = new DoubleRankAPSSVMPegasos();
					//DoubleRankAPSSVMCuttingPlane1Slack classifier = new DoubleRankAPSSVMCuttingPlane1Slack();
					classifierRank.setLambda(1e-4);
					classifierRank.setMaxIterations(50);

					classifierRank.train(train);
					double ap = classifierRank.averagePrecision(test);
	...
	public static List<TrainingSample<double[]>> concatenation(List<List<double[]>> listScores, List<STrainingSample<BagImageRegion,Integer>> listBag) {
