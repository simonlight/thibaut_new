 ...
 *
 */
public class CombinaisonObjectBankLinearMantraM2PlattWithoutDifficultVeryDeep {
	...
	public static void main(String[] args) {
		...
		double[] cCV = {Double.parseDouble(args[3])};
		//int[] epochsCV = {5, 10, 50, 100, 200, 500, 1000};
		int[] epochsCV = {Integer.parseInt(args[4])};

		double[] aCV = {Double.parseDouble(args[2])};
		double[] bCV = {0.};
		...
		for(int split : splitCV) {
			...
			String scoreDir = simDir + "/ICCV15/scoresMaxMin/MANTRA/M2/AP/CuttingPlane1Slack/imagenet-vgg-verydeep-19_layer_" + layer + "/BagImageRegion/without_difficults/";
			String inputDir = simDir + "/files_BagImageRegion/without_difficults/";

			List<STrainingSample<BagImageRegion, Integer>> listTrain = BagReader.readBagImageRegion(inputDir + "/" + cls + "_" + trainset + "_imagenet-vgg-verydeep-19_layer_" + layer + "_scale_100.txt", numWords, true, true, null, false, 0);
			List<STrainingSample<BagImageRegion, Integer>> listTest = BagReader.readBagImageRegion(inputDir + "/" + cls + "_" + testset + "_imagenet-vgg-verydeep-19_layer_" + layer + "_scale_100.txt", numWords, true, true, null, false, 0);
