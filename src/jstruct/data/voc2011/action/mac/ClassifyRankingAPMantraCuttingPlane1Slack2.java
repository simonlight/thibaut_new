...
public class ClassifyRankingAPMantraCuttingPlane1Slack2 {
	...
	public static void main(String[] args) {
		...
		double[] bestLambda = new double[VOC2011.getActionClasses().length];
		...
		for(int iCls=0; iCls<VOC2011.getActionClasses().length; iCls++) {
			...
			for(double epsilon : epsilonCV) {
				for(double lambda : lambdaCV) {
					double[] clsResults = new double[splitCV.length]; 
					for(int split : splitCV) {			
						...
						if(fileClassifier != null) {
							...
							clsResults[split-1] = ap;
						}
					}
					
					if(mapAll[iCls] < VectorOp.mean(clsResults)) {
						mapAll[iCls] = VectorOp.mean(clsResults);
						bestLambda[iCls] = lambda;
						map[iCls] = clsResults;
					}
				}
			}
		...
		System.out.println("best lambda " + Arrays.toString(bestLambda));
	}
