...
/**
 * <b>Cutting-Plane Training of Structural SVMs</b> <br/>
 * Thorsten Joachims, Thomas Finley, and Chun-Nam John Yu<br/>
 * <i>Machine Learning (2009)</i>
 * </p>
 * 
 * @author Thibaut Durand - durand.tibo@gmail.com
 *
 */
public abstract class SSVMCuttingPlane1Slack<X,Y> extends SSVM<X,Y> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6379292674359616129L;
	...
	protected void learning(List<STrainingSample<X, Y>> l) {
		...
		while(t<cpmin || (t<=cpmax && linear.valueOf(w,gt) < ct - xi - epsilon)) {
			...
			if(gram != null) {
				...
				for(int i=0; i<lc.size(); i++) {
					gram[lc.size()-1][i] = linear.valueOf(lg.get(lc.size()-1), lg.get(i));
				...
				for(int i=0; i<gram.length; i++) {
					for(int j=i; j<gram.length; j++) {
						gram[i][j] = linear.valueOf(lg.get(i), lg.get(j));
						gram[j][i] = gram[i][j];
						if(i==j) {
							gram[i][j] += 1e-8;
						}
			...
			for(int i=0; i<alphas.length; i++) {
				for(int d=0; d<lg.get(i).length; d++) {
		...
		System.out.println("*");
		if(verbose >= 0) {
			System.out.println("iterations= " + t + "\tloss aumented inference time= " + lossAugmentedTime/1000 + "s \t solver time= " + solverTime/1000 + "s");
		}
	}
	 ...
	 * @return
	 */
	protected Object[] cuttingPlane(final List<STrainingSample<X, Y>> l, final double[] w) {
			...
			for(int i=0 ; i<l.size(); i++) {
				...
				futures.add(completionService.submit(new Callable<Object[]>() {
					...
					public Object[] call() throws Exception {
						STrainingSample<X, Y> ts = l.get(ii);
