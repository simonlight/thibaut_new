 ...
 *
 */
public abstract class LSSVMCuttingPlane1Slack<X,Y,H> extends LSSVM<X,Y,H> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5105670685114278775L;
	 ...
	 */
	protected void trainOneCCCPIteration(List<STrainingSample<LatentRepresentation<X,H>,Y>> l) {
		...
		if(nThreads > 1) {
			...
			for(int i=0 ; i<l.size(); i++) {
				...
				futures.add(completionService.submit(new Callable<Object[]>() {
					...
					public Object[] call() throws Exception {
						STrainingSample<LatentRepresentation<X,H>,Y> ts = l.get(ii);			
			...
			for(Future<Object[]> f : futures) {
				try {
					...
					H hp = (H)res[1];
					ct += (double)res[2];
					STrainingSample<LatentRepresentation<X,H>,Y> ts = l.get((int) res[3]);
	...
	protected void showParameters() {
		...
		System.out.println("epsilon= " + epsilon + "\tmaxCCCPIter= " + maxCCCPIter + "\tminCCCPIter= " + minCCCPIter 
