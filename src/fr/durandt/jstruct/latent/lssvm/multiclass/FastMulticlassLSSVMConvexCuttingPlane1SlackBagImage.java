 ...
 *
 */
public class FastMulticlassLSSVMConvexCuttingPlane1SlackBagImage extends FastMulticlassLSSVMConvexCuttingPlane1Slack<BagImage,Integer> {
	...
	protected Object[] predictionOutputLatent(BagImage x, double[][] w) {
		...
		for(int y : listClass) {	// For each class
			for(int h=0; h<x.numberOfInstances(); h++) {	// For each latent
				...
				System.out.println("y="+y+"\th="+h+"\tval="+val);
				if(val>valmax){
					valmax = val;
					ypredict = y;
					hpredict = h;
