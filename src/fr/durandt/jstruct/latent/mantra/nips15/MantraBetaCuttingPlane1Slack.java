 ...
 *
 */
public abstract class MantraBetaCuttingPlane1Slack<X,Y,H> extends MantraBeta<X,Y,H> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6262716234772882067L;
	...
	public Object[] cuttingPlane(final List<STrainingSample<LatentRepresentation<X,H>,Y>> l, final double[] w) {
		...
		for(int i=0; i<l.size(); i++){
			...
			for(int d=0; d<dim; d++) {
				gt[d] += -psi1[d] - beta*psi2[d] + psi3[d] + beta*psi4[d];
			}
		}
		ct /= n;
	...
	public String toString() {
		String s = "mantra_beta_cuttingplane1slack_optim_" + optim + "_beta_" + beta + "_lambda_" + lambda 
