 ...
 * 
 */
package fr.durandt.jstruct.latent.mantra.nips15.multiclass;
...
import fr.durandt.jstruct.latent.mantra.nips15.MantraBetaCuttingPlane1Slack;
 ...
 *
 */
public class MulticlassMantraBetaCuttingPlane1SlackBagImageRegion extends MantraBetaCuttingPlane1Slack<BagImageRegion, Integer, Integer> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4027899944932657377L;
	...
	protected Object[] lossAugmentedInference(STrainingSample<LatentRepresentation<BagImageRegion, Integer>, Integer> ts, double[] w) {
		...
		for(int y : listClass) {	// For each class
			...
			double vmax = (double) or[1];
			double vmin = (double) or[3];
			double val = vmax + beta*vmin + delta(ts.output, y, (Integer) or[0]);
			if(val>valmax){
				valmax = val;
				ypredict = y;
				hppredict = (Integer) or[0];
