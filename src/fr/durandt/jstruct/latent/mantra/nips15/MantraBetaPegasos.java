 ...
 *
 */
public abstract class MantraBetaPegasos<X,Y,H> extends MantraBeta<X,Y,H> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6191414775466692674L;
	...
	public void trainOneEpoch(List<STrainingSample<LatentRepresentation<X,H>,Y>> l, boolean lastIteration, int s0) {
		...
		for(int s=0; s<l.size(); s++) {
			...
			double loss = delta(ts.output, yp, hp) + valueOf(ts.input.x, yp, hp) + beta*valueOf(ts.input.x, yp, hm) 
			...
			if(loss > 0) {
				...
				for(int d=0; d<w.length; d++) {
					w[d] += (psi3[d] + beta*psi4[d] - psi1[d] - beta*psi2[d])*eta;
				}
			}
	...
	public String toString() {
		String s = "mantra_beta_pegasos_lambda_" + lambda + "_maxIter_" + maxIter + "_beta_" + beta;
		return s;
	}
