...
/**
 * Abstract class for MANTRA model used for ICCV 2015 submission
 * 
 ...
 * @param <H> latent
 */
public abstract class Mantra<X,Y,H> implements LatentStructuralClassifier<X,Y,H> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7624735276583559568L;
	 ...
	 * yp = argmax_y max_h &lt w, \psi(x,y,h) &gt + min_h &lt w, \psi(x,y,h) &gt <br/>
	...
	protected double loss(final List<STrainingSample<LatentRepresentation<X,H>,Y>> l, final double[] w) {
		...
		if(nThreads > 1) { // Multi-threads execution
			...
			for(int i=0 ; i<l.size(); i++) {
				...
				futures.add(completionService.submit(new Callable<Double>() {
					...
					public Double call() throws Exception {
						...
						return delta(ts.output, yp, hp) + valueOf(ts.input.x, yp, hp) + valueOf(ts.input.x, yp, hm)
			...
			for(STrainingSample<LatentRepresentation<X,H>,Y> ts : l) {
				...
				// Compute the loss with the predicted output and latent variables
				loss += delta(ts.output, yp, hp) + valueOf(ts.input.x, yp, hp) + valueOf(ts.input.x, yp, hm)
						- valueOf(ts.input.x, ts.output, hpi) - valueOf(ts.input.x, ts.output, hmi);
			}
		}
		loss /= l.size();
