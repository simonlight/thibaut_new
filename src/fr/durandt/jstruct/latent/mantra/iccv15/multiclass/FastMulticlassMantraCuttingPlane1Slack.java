 ...
 *
 */
public abstract class FastMulticlassMantraCuttingPlane1Slack<X,H> extends FastMulticlassMantra<X,H> {
	...
	public Object[] cuttingPlane(final List<STrainingSample<LatentRepresentation<X,H>,Integer>> l, 
		...
		for(int i=0; i<l.size(); i++){
			...
			H hmi = precomputedScore.getHmin(ts.output);

			double[] psi3 = psi(ts.input.x, hpi);
			double[] psi4 = psi(ts.input.x, hmi);

			double score = delta(ts.output, yp, hp) + precomputedScore.getVmax(yp) + precomputedScore.getVmin(yp)
					- precomputedScore.getVmax(ts.output) - precomputedScore.getVmin(ts.output);
	...
	public String toString() {
		String s = "fast_multiclass_mantraM2_cuttingplane1slack_optim_" + optim + "_lambda_" + lambda 
